...
def getStockQuotes(symbols, source, startDate, endDate):
    ...
    for symbol in symbols:
        quotes[symbol] = web.DataReader(
            symbol, data_source=source, start=startDate, end=endDate)['Adj Close']
...
def findMaxSharpeRatioPortfolio(meanReturns, covMatrix, riskFreeRate):
    ...
    opts = sco.minimize(negSharpeRatio, numAssets*[
                        1./numAssets, ], args=args, method='SLSQP', bounds=bounds, constraints=constraints)
...
def findMinVariancePortfolio(meanReturns, covMatrix):
    ...
    opts = sco.minimize(getPortfolioVol, numAssets*[
                        1./numAssets, ], args=args, method='SLSQP', bounds=bounds, constraints=constraints)
...
def findEfficientFrontier(meanReturns, covMatrix, rangeOfReturns):
    ...
    for ret in rangeOfReturns:
        efficientPortfolios.append(
            findEfficientReturn(meanReturns, covMatrix, ret))

    return efficientPortfolios


availStocks = ['GOOGL', 'AAPL', 'AMZN',
               'MSFT', 'F', 'BMW.DE', 'TM', 'KO', 'PEP']
stocks = availStocks
...
efficientPortfolios = findEfficientFrontier(
    meanDailyReturn, covariance, targetReturns)
plt.plot([p['fun']*np.sqrt(numPeriodsAnnually)
         for p in efficientPortfolios], targetReturns*numPeriodsAnnually, marker='x')

# Find portfolio with maximum Sharpe ratio
maxSharpe = findMaxSharpeRatioPortfolio(
    meanDailyReturn, covariance, riskFreeRate)
rp, sdp = calcPortfolioPerf(maxSharpe['x'], meanDailyReturn, covariance)
plt.plot(sdp*np.sqrt(numPeriodsAnnually), rp *
         numPeriodsAnnually, 'r*', markersize=15.0)
...
plt.plot(sdp*np.sqrt(numPeriodsAnnually), rp *
         numPeriodsAnnually, 'y*', markersize=15.0)
